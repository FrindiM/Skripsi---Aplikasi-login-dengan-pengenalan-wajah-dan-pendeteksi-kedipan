    ...
    if ((name.isalpha()) or (' ' in name)):
        if (Id.isnumeric()):
            ...
            if(Id.isnumeric()==False):
                ress = "Masukan Id yang benar"
                message.configure(text=ress)
                mess._show(title='Id salah', message='Masukan Id yang benar (hanya angka)')
    else:
        if (name.isalpha() == False):
            res = "Masukan Nama yang benar"
            message.configure(text=res)
            mess._show(title='Nama salah', message='Masukan Nama yang benar (hanya alfabet)')
...
def halaman3():
    ...
    if bb == 'Unknown':
        ...
        fr_head1.place(x=0,y=0,relwidth=1)
        kembali = tkinter.Button(frame3, text="Logout", command=halaman1, fg="black", bg="#00aeff", width=34, height=1, activebackground = "white", font=('times', 16, ' bold '))
